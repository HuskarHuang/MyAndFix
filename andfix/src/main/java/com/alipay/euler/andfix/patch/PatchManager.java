...
public class PatchManager {
	 ...
	 * patchs
	 */
	private final SortedSet<com.alipay.euler.andfix.patch.Patch> mPatchs;
	/**
	 * classloaders
	 */
	...
	public PatchManager(Context context) {
		...
		mPatchDir = new File(mContext.getFilesDir(), DIR);//创建修复包存放的沙盒目录
	...
	private Patch addPatch(File file) {
		com.alipay.euler.andfix.patch.Patch patch = null;
	...
	public void addPatch(String path) throws IOException {
		...
		File dest = new File(mPatchDir, src.getName());//复制到/data/user/0/com.hwh.myandfix/files/apatch/out.apatch
		if(!src.exists()){
			throw new FileNotFoundException(path);
		}
		if (dest.exists()) {
			Log.i(TAG, "patch [" + path + "] has be loaded.");
			return;
		}
