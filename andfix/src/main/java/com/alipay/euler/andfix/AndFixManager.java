...
public class AndFixManager {
	...
	public AndFixManager(Context context) {
		mContext = context;
		mSupport = Compat.isSupport();//一件事是判断当前环境是否支持热修复，一件事是初始化修复包安全校验的工作
		if (mSupport) {
			mSecurityChecker = new SecurityChecker(mContext);//检查修复包的签名安全
			mOptDir = new File(mContext.getFilesDir(), DIR);//apatch_opt
		...
		try {
			...
			ClassLoader patchClassLoader = new ClassLoader(classLoader) {
				...
				protected Class<?> findClass(String className)
					...
					Log.e(TAG, "findClass className: " + className);
