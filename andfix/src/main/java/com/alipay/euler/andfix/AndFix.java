...
public class AndFix {
	...
	private static void initFields(Class<?> clazz) {
		...
		for (Field srcField : srcFields) {
			Log.i(TAG, "modify " + clazz.getName() + "." + srcField.getName()
					+ " flag:");
			setFieldFlag(srcField);//改为public
		}
	}
	...
	public static boolean setup() {
		try {
			final String vmVersion = System.getProperty("java.vm.version");//2.1.0
			boolean isArt = vmVersion != null && vmVersion.startsWith("2");
			int apilevel = Build.VERSION.SDK_INT;
			Log.i(TAG, "setup: vmVersion = " + vmVersion + ", apilevel = " + apilevel);//22
