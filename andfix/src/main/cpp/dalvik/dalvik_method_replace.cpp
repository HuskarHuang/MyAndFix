...
extern jboolean __attribute__ ((visibility ("hidden"))) dalvik_setup(
	...
	if (dvm_hand) {
		...
		jClassMethod = env->GetMethodID(clazz, "getDeclaringClass",
						"()Ljava/lang/Class;");//调用dest的 Method.getDeclaringClass方法获取method的类对象clazz。
...
extern void __attribute__ ((visibility ("hidden"))) dalvik_replaceMethod(
	...
	jobject clazz = env->CallObjectMethod(dest, jClassMethod);//class对象
	ClassObject* clz = (ClassObject*) dvmDecodeIndirectRef_fnPtr(
			dvmThreadSelf_fnPtr(), clazz);////将clazz对象转换为dalvik直接引用,返回ClassObject*引用它.因为davlik是操作ClassObject的。
	LOGD("class name: %s", clz->descriptor);
	LOGD("class initstatus: %d", clz->status);
